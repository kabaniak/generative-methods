...
classes: [Feathers],
currentClass: Feathers,
xaxis: Feathers.labels[0],
yaxis: Feathers.labels[1],
...
draw(p) {
  ...
  individualsSorted.forEach((individual, index) => {
    ...
    if (app.hovered === individual) {
      p.stroke(...individual.aof.idColor, .15)
      p.strokeWeight(20)
    }
    if (app.selected === individual) {
      p.stroke(...individual.aof.idColor, .7)
      p.strokeWeight(20)
    }
    if (app.shiftSelected.includes(individual)) {
      p.stroke(...individual.aof.idColor, .5)
      p.strokeWeight(20)
    }

    p.fill(...individual.aof.idColor, .6)
    p.ellipse(0, 0, 50, 30)
